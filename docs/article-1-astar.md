...
On the 8th of December 2025, a set of driving commands arrived at a rover parked on the rim of a crater on Mars. Nothing about the commands looked unusual. The rover, Perseverance, read them, checked them against its surroundings, and drove 210 metres across a field of rocks and sand ripples without a single human steering it.

What made that day historic wasn't the distance. It was the author. For the first time in the history of spaceflight, the route hadn't been drawn by a human rover driver. It had been drawn by an AI — Anthropic's Claude — which had studied orbital images of the terrain, picked out the boulders and sand traps to avoid, and strung together a chain of waypoints into a safe path. Two days later it did it again, this time for 246 metres. The plans were checked by human engineers against a simulation that models over 500,000 variables of the rover's behaviour, and they held up with only minor changes.

That moment is the reason I'm writing this series. Because to appreciate what it means for an AI to plan a path on Mars, you have to understand what "planning a path" has meant for the last half-century — and that story starts with an algorithm from 1968 that is, remarkably, still at the heart of how robots find their way today.
...
Mars is, on average, about 225 million kilometres from Earth. Radio waves take between three and twenty-two minutes to make that trip one way, depending on where the two planets are in their orbits. That delay makes real-time driving — what JPL engineers call "joy-sticking" — completely impossible. By the time you saw a rock through the rover's eyes and told it to stop, it would already have driven into the rock several minutes ago.
...
This constraint — plan carefully in advance, because you cannot correct in the moment — is exactly the kind of problem that *path planning algorithms* were invented to solve. The rover needs to find a route that is not just any route, but a good one: short, safe, and cheap in terms of the precious energy it spends. And it needs to be found by a computer following a precise procedure, not by intuition.
...
The answer is that the map is built, painstakingly, from images — and it comes from two directions at once.
...
From the **ground**, the rover's own cameras photograph its immediate surroundings in stereo — two cameras slightly apart, exactly like your two eyes — which lets onboard software reconstruct the 3D shape of the terrain nearby, judging what's a safe step and what's a wheel-swallowing trap.

Both of these get distilled into something a computer can plan on: a **grid**. The terrain is divided into cells, like a chessboard laid over the landscape, and each cell is tagged with a number — a *cost* — that says how hard or dangerous it is to drive across. Flat bedrock is cheap. A slope is more expensive. A boulder is effectively infinite: never go there.

This grid, with its per-cell costs, is called a **traversability grid** or a **cost map**, and it is the stage on which every algorithm in this series performs. It's worth pausing on the honesty of this abstraction: the real Martian surface is continuous, messy, and three-dimensional, and we are flattening it into a checkerboard of numbers. Almost everything clever in path planning comes from how well — or how badly — that checkerboard captures reality. We will spend the whole series pushing on that idea. For now, take the grid as given: a field of cells, each with a cost, a starting cell where the rover sits, and a goal cell we want it to reach.
...
The obvious question is: could we *aim*? If we know roughly which direction the goal is in, could we bias the search to explore toward it first, and leave the wrong-way cells for later — or never?
...
- **h** — the *heuristic*: an estimate, a guess, of the cost from this cell onward to the goal. We haven't travelled it yet, so we can only estimate.
...
For a grid where you can only step north, south, east, or west, the natural admissible heuristic is the **Manhattan distance**: the number of horizontal steps plus the number of vertical steps to the goal, ignoring obstacles. It's called that because it's how you'd measure distance in a city laid out in a grid, walking the blocks — you can't cut diagonally through buildings. Since it ignores obstacles, it can only ever *underestimate* the real cost (obstacles make things longer, never shorter), which is precisely the optimism A\* needs.

If you set the heuristic to zero — refuse to aim at all — A\* stops aiming and collapses back into Dijkstra, exploring blindly in all directions. That's a lovely way to feel what the heuristic buys you: the same algorithm, with and without a sense of direction.
...
![A* frontier expanding across Mars terrain](search.gif)
...
![A* final path over explored terrain](hero.png)
...
And you need one genuinely clever data structure: a **priority queue** that always hands you the frontier cell with the smallest **f**, and — this is the subtle part — lets you *update* a cell's priority when you discover a cheaper route to it mid-search. Get that structure right and A\* sings; get it wrong and it either returns wrong answers or crawls. It's the kind of unshowy component that separates a toy implementation from one you'd trust, and we'll lean on it for every algorithm in the series.
...
So how does a 1968 algorithm connect to an AI planning drives on Mars in 2025?

A\* by itself has a quiet assumption baked into it: that the map doesn't change. It plans across a grid it knows completely, from start to goal, once. But a rover creeping across an alien landscape is *constantly* learning that the map was wrong — a rock it couldn't see from orbit, a patch of sand softer than it looked. Replanning from scratch every time it learns something new would be far too slow. The algorithms that followed A\* — D\*, D\* Lite, Field D\* — are, in large part, the story of teaching A\*'s core idea to *cope with a map that keeps changing underfoot*. That's part two.

And the December 2025 drives? They didn't replace any of this. The AI's job was to look at raw orbital imagery — the actual pictures, with all their visual richness that no cost-grid can fully capture — and decide *where the good waypoints are*: this corridor between the boulders, that gap in the ripples. It handled the perception and the high-level route sketching that humans used to do by eye. The rover's onboard navigation, descended from exactly the algorithms in this series, still handled the metre-by-metre execution between those waypoints. As JPL's Vandi Verma put it, the promise of AI here is in streamlining the three pillars of off-world driving: *perception* (seeing the rocks and ripples), *localization* (knowing where you are), and *planning and control* (deciding and executing the safest path).

That last pillar — planning and control — is where our story lives. Sixty years of it, from a Stanford research paper to the rim of Jezero Crater. We've just met the ancestor. Next, we teach it to handle a world that refuses to hold still.
